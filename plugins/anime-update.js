  ...
  if (chat && chat.updateAnime) {
    ...
    setInterval(async () => {
      ...
      if (lastAnime.includes(title))
        return conn.logger.info(`${title} already sent to "${m.chat}"`);
      let length = lastAnime[lastAnime.length - 1];
      lastAnime.push(title);
      if (lastAnime.indexOf(length) !== -1)
        lastAnime.splice(lastAnime.indexOf(length), 1);
      conn.logger.info(`Sending anime ${title} to "${m.chat}"`);
      let detailAnime = await getDetailAnime(url),
        download = detailAnime.download;
      let txt = parseResult(detailAnime, {
        title: "*ANIME UPDATE*",
        ignoreKey: ["update", "cover", "download"],
      });
      let list = Object.keys(download).map((v) =>
        parseResult(download[v], {
          headers: `*• ${v}:*`,
          body: " - Quality: %key\n - Url: %value",
        })
      );
      ...
      let quoted = await conn.sendMessage(m.chat, {
        image: { url: cover },
        caption: `${txt}\n*• Download:*\n${list.join("\n")}`,
        footer: detailAnime.update,
        templateButtons,
      });
      if (/Movie/.test(detailAnime.episode))
        return conn.reply(
          m.chat,
          "Bot tidak dapat mengirim file video karena terlalu besar...",
          quoted
        );
      let res = await downloadAnime(
        zippy?.["480p"] || zippy?.["720p"] || zippy?.["360p"]
      ).catch(() => null);
      if (!res)
        return conn.reply(m.chat, "Link download belum tersedia...", quoted);
...
async function getDetailAnime(url) {
  ...
  let $ = cheerio.load(html),
    obj = {};
  obj.title = $("div.pagetitle > h1")
    .text()
    .trim()
    .replace(/Subtitle Indonesia/, "");
  obj.episode = /Episode/.test(obj.title)
    ? obj.title.split(" Episode ")[1]
    : "Movie";
  ...
  obj.sinopsis =
    $("div.contentdeks").text().trim() || $("div.unduhan").eq(0).text().trim();
