...
let handler = async (m, { conn, text, usedPrefix }) => {
  ...
  if (now < global.db.data.users[hl[0]].bannedDate)
    global.db.data.users[hl[0]].bannedDate += jumlahHari;
  else global.db.data.users[hl[0]].bannedDate = now + jumlahHari;
  conn.reply(
    m.chat,
    `*TERBANNED*\n\nLaporkan keluhan mu ke website kami di bagian Customer service dengan cara ketik .website\n\nKamu di banned *@${
      hl[0].split("@")[0]
    }* selama *${hl[1]} hari*.\n\n*Banned : ${msToDate(
      global.db.data.users[hl[0]].bannedDate - now
    )}*`,
    m,
