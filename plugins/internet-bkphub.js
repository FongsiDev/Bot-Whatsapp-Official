...
let handler = async (m, { conn, usedPrefix, text, args, command }) => {
  if (command == "caribokep") {
    ...
    let json = await fetch(
      `https://bx-hunter.herokuapp.com/api/xvideosearch?query=${text}&apikey=FuckBitch`
    );
...
`;
  ...
  if (command == "caribokep1") {
    if (!text) throw `Contoh penggunaan ${usedPrefix}${command} japan`;
    let json = await fetch(
      `https://bx-hunter.herokuapp.com/api/pornhubscraper?query=${text}&apikey=FuckBitch`
    );
    let jsons = await json.json();
    let caption = `*⎔┉━「 ${command} 」━┉⎔*`;
    for (let x of jsons.result) {
...
`;
  ...
  if (command == "dlbokep") {
    ...
    let json = await fetch(
      `https://bx-hunter.herokuapp.com/api/xvideodetail?url=${text}&apikey=FuckBitch`
    );
    let jsons = await json.json();
    let x = jsons.result;
