...
let handler = async (m, { conn, usedPrefix, text, args, command }) => {
  if (command == "gettenor") {
    ...
    let gas = await fetch(
      `https://g.tenor.com/v1/search?q=${text1}&key=LIVDSRZULELA&limit=${text2}`
    );
  ...
  if (command == "getteles") {
    if (!text)
      throw `Contoh penggunaan ${usedPrefix}${command} https://t.me/addsticker/...`;
    let gas = await fetch(
      `https://api-xfar05.herokuapp.com/api/telesticker?url=${text}`
    );
    let json = await gas.json();
    let ter = json.result;
    let stiker = await sticker(
      null,
      global.API(ter.getRandom()),
      global.packname,
      global.author
    );
