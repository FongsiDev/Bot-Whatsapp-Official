...
let handler = async (m, { conn, args }) => {
  ...
  let res = await fetch(
    "https://expressjs-akkun.up.railway.app/nhentai?code=" + code
  );
  if (!res.ok) throw await res.statusText;
  let json = await res.json();
  let buffer = await (
    await fetch(
      "https://external-content.duckduckgo.com/iu/?u=" + json.result.pages[0]
    )
  ).buffer();
  ...
  conn.sendMessage(
    ...
    {
      document: {
        url: "https://expressjs-akkun.up.railway.app/nhentai/" + code,
      },
      jpegThumbnail,
