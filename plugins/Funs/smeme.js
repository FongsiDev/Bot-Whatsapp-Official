...
let handler = async (m, { conn, text, usedPrefix, command }) => {
  ...
  try {
    ...
    let meme = `https://api.memegen.link/images/custom/${encodeURIComponent(
      atas ? atas : ""
    )}/${encodeURIComponent(bawah ? bawah : "")}.png?background=${out}`;
    stiker = await sticker(false, meme, global.packname, global.author);
    if (stiker)
      await conn.sendFile(m.chat, stiker, "sticker.webp", "", m, null, adReply);
    else
      m.reply(
...
handler.help = handler.command = [
  "smeme2 (caption|reply media)",
  "smm2 <url>",
  "sm2(caption|reply media)",
];
