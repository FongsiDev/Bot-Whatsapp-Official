...
`.trim();
  ...
  if (!listItems[item])
    return conn.sendButton(m.chat, text, footer, image, buttons, m);
...
`.trim(),
        ...
        [
          [
            `ᴀsᴋ ᴛᴏ ᴀʟʟ`,
            `${usedPrefix}tagall sᴏᴍᴇʙᴏᴅʏ ᴩʟᴇᴀsᴇ sᴇɴᴅ *${
              listItems[item][paymentMethod] * total - user[paymentMethod]
            }* ${global.rpg.emoticon(paymentMethod)}${paymentMethod} ᴛᴏ ᴍᴇ.
⮕ ᴛᴏ ᴛʀᴀɴsғᴇʀ ${paymentMethod}:
${usedPrefix}transfer ${paymentMethod} ${
              listItems[item][paymentMethod] * total - user[paymentMethod]
