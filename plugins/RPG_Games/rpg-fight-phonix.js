let handler = async (m, { conn, usedPrefix, participants }) => {
  ...
  m.reply(
    `*Pet Kamu* (🦅phonix ${
      global.db.data.users[m.sender].phonix
    }) ⚔️menantang 🦅phonixnya *${conn.getName(lawan)}* (🦅phonix ${
      global.db.data.users[lawan].phonix
  ...
  let kesempatan = [];
  for (i = 0; i < global.db.data.users[m.sender].phonix; i++)
    kesempatan.push(m.sender);
  for (i = 0; i < global.db.data.users[lawan].phonix; i++)
  ...
  if (pointPemain > pointLawan) {
    ...
    m.reply(
      `*${conn.getName(m.sender)}* [${pointPemain * 10}] - [${
        ...
        global.db.data.users[m.sender].phonix
      }) MENANG melawan 🦅phonixnya *${conn.getName(lawan)}* (phonix ${
        global.db.data.users[lawan].phonix
      }) karena phonix🦅kamu ${
    ...
    m.reply(
      `*${conn.getName(m.sender)}* [${pointPemain * 10}] - [${
        ...
        global.db.data.users[m.sender].phonix
      }) KALAH melawan 🦅phonixnya *${conn.getName(lawan)}* (phonix ${
        global.db.data.users[lawan].phonix
      }) karena pet kamu ${
