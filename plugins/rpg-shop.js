...
${Object.keys(listItems)
    .map(v => {
      ...
      return `${global.rpg.emoticon(v)}${v} | ${
    ...
    if (user[paymentMethod] < listItems[item][paymentMethod] * total)
      return m.reply(
        `You don't have enough ${global.rpg.emoticon(
          paymentMethod
        )}${paymentMethod} to buy *${total}* ${global.rpg.emoticon(
          item
        )}${item}. You need *${listItems[item][paymentMethod] * total -
          user[paymentMethod]}* more ${paymentMethod} to be able to buy`
      );
