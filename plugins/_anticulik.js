  ...
  if (chat.autoReply) {
    ...
    if (
      (m.mtype === "groupInviteMessage" ||
        m.text.startsWith("https://chat.whatsapp.com") ||
    ...
    if (m.mtype === "reactionMessage") {
      let caption = `Terdeteksi ${name} @${
        who.split("@")[0]
      } Lagi Mengirim Reaction [ ${m.text} ]`;
      this.sendButton(
        m.chat,
        caption,
        wm,
        ...
        [
          [
            user.banned ? "Pemilik Bot" : "Menu",
            user.banned ? "/owner" : "/menu",
          ],
        ],
        m,
    ...
    if (/^(aktif|w(ey|oi)|bot|ha[iy]|we|oy|p)$/i.test(m.text)) {
      ...
      let caption = `*${apsih.getRandom()}* kak ${name} @${
        who.split("@")[0]
      } 🗿`;
      this.sendButton(
        m.chat,
