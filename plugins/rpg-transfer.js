...
`.trim();
  ...
  if (user[type] * 1 < count)
    return m.reply(
      `Your *${rpg.emoticon(type)}${type}${special(type)}* is less *${count -
        user[type]}*`
    );
  let confirm = `
Are you sure you want to transfer *${count}* ${rpg.emoticon(
    type
  )}${type}${special(type)} to *@${(who || "").replace(
    /@s\.whatsapp\.net/g,
    ""
  )}*
Timeout *60* detik
`.trim();
  let c = "©games-wabot";
  conn.sendButton(m.chat, confirm, c, null, [["y"], ["n"]], m, {
    mentions: [who]
  });
  confirmation[m.sender] = {
    ...
    count,
    timeout: setTimeout(
      () => (m.reply("Timeout"), delete confirmation[m.sender]),
      60 * 1000
    )
  };
}
...
handler.before = async m => {
  ...
  if (/y(es)?/g.test(m.text.toLowerCase())) {
    ...
    if (previous > user[type] * 1 && _previous < _user[type] * 1)
      m.reply(
        `Succes transfer *${count}* ${rpg.emoticon(type)}${type}${special(
          type
        )} to *@${(to || "").replace(/@s\.whatsapp\.net/g, "")}*`,
        null,
        { mentions: [to] }
      );
    else {
      ...
      m.reply(
        `Failted to transfer *${count}* ${rpg.emoticon(type)}${type}${special(
          type
        )} to *@${(to || "").replace(/@s\.whatsapp\.net/g, "")}*`,
        null,
        { mentions: [to] }
