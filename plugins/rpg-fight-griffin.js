let handler = async (m, { conn, usedPrefix, participants }) => {
  ...
  m.reply(
    `*Pet Kamu* (🦚griffin ${
      global.db.data.users[m.sender].griffin
    }) ⚔️menantang 🦚griffinnya *${conn.getName(lawan)}* (🦚griffin ${
      global.db.data.users[lawan].griffin
  ...
  let kesempatan = [];
  for (i = 0; i < global.db.data.users[m.sender].griffin; i++)
    kesempatan.push(m.sender);
  for (i = 0; i < global.db.data.users[lawan].griffin; i++)
  ...
  if (pointPemain > pointLawan) {
    ...
    m.reply(
      `*${conn.getName(m.sender)}* [${pointPemain * 10}] - [${
        ...
        global.db.data.users[m.sender].griffin
      }) MENANG melawan 🦚griffinnya *${conn.getName(lawan)}* (griffin ${
        global.db.data.users[lawan].griffin
      }) karena griffin🦚kamu ${
    ...
    m.reply(
      `*${conn.getName(m.sender)}* [${pointPemain * 10}] - [${
        ...
        global.db.data.users[m.sender].griffin
      }) KALAH melawan 🦚griffinnya *${conn.getName(lawan)}* (griffin ${
        global.db.data.users[lawan].griffin
      }) karena pet kamu ${
