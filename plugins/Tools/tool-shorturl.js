...
let handler = async (m, { conn, usedPrefix, args }) => {
  ...
  if (args[1] == "linkpoi") {
    ...
    m.reply(pros).then((_) =>
      conn.reply(m.chat, `${tesk}${poi.shorturl.replace("/", "/")}`, m)
    );
  }
  //------------
  ...
  if (args[1] == "bitly") {
    let bit = await (
      await fetch(
        `https://api.xteam.xyz/shorturl/bitly?url=${args[0]}&APIKEY=ebb6251cc00f9c63`
      )
    ).json();
    m.reply(pros).then((_) =>
      conn.reply(m.chat, `${tesk}${bit.result.link}`, m)
    );
  }
  //------------
  ...
  if (args[1] == "ouo") {
    let ouo = await (
      await fetch(
        `https://api.lolhuman.xyz/api/ouoshortlink?apikey=e54205a4ca2caa368cc067bb&url=${args[0]}`
      )
    ).json();
