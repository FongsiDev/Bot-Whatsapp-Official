    ...
    try {
        ...
        try {
            ...
            if (settings) {
                ...
                if (!('templateDocument' in settings)) settings.templateDocument = false
            ...
            if (datas) {
                ...
                if (!('openaipc' in datas)) 
                    datas.openaipc = false
                if (!('prems' in datas)) 
        ...
        for (let name in global.plugins) {
            ...
            if ((usedPrefix = (match[0] || '')[0])) {
                ...
                if (m.chat in global.db.data.chats || m.sender in global.db.data.users) {
                    let chat = global.db.data.chats[m.chat]
                    let user = global.db.data.users[m.sender]
                    if (name != "owner-unbanchat.js" && name != "tool-delete.js" && chat?.isBanned)
                        return // Except this
                ...
                for (let user of participants) {
                    ...
                    try {
                        ...
                        text = (action === 'add' ? (chat.sWelcome || this.welcome || conn.welcome || 'Welcome, @user').replace('@subject', await this.getName(id)).replace('@desc', groupMetadata.desc?.toString() || 'unknow') :
                            ...
                            const image = await new knights.Welcome().setUsername(nameusr).setGuildName(gcname).setGuildIcon(ppgc).setMemberCount(memcount).setAvatar(ppmem).setBackground("https://telegra.ph/file/aa9ef9dde5ea24dd88476.jpg").toAttachment();
                            let wel = await this.resize(image.toBuffer(), 300, 150)
                            const imagelea = await new knights.Goodbye().setUsername(nameusr).setGuildName(gcname).setGuildIcon(ppgc).setMemberCount(memcount).setAvatar(ppmem).setBackground("https://telegra.ph/file/aa9ef9dde5ea24dd88476.jpg").toAttachment();
...
let tag = `@${m.sender.replace(/@.+/, '')}`
  ...
  if (msg) return conn.reply(m.chat, `Hai ${tag},\n${msg}`, fkontak, { mentions: [m.sender] })
}
