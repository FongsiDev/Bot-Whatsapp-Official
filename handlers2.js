import { smsg } from "./lib/simple2.js";
  ...
  try {
    ...
    try {
      ...
      if (user) {
        ...
        if (!isNumber(user.lastbinis)) user.lastbisnis = 0;
        ...
        if (!isNumber(user.masak)) user.masak = 0;
        if (!isNumber(user.masakrole)) user.masakrole = 0;
        if (!isNumber(user.masakexp)) user.masakexp = 0;
        if (!isNumber(user.masaklevel)) user.masaklevel = 0;
      ...
      if (settings) {
        ...
        if (!("templateGif" in settings)) settings.templateGif = false;
        if (!("templateMsg" in settings)) settings.templateMsg = false;
        if (!("templateDocument" in settings))
          settings.templateDocument = false;
        if (!("templateLoc" in settings)) settings.templateLoc = false;
      } else
        global.db.data.settings[this.user.jid] = {
          self: false,
          autoread: true,
      ...
      if (datas) {
        ...
        if (!("akinator" in datas)) datas.akinator = {};
        if (!("payment" in datas)) datas.payment = [];
        if (!("miku" in datas)) datas.miku = [];
        if (!("domain" in datas)) datas.domain = "";
        if (!("apikey" in datas)) datas.apikey = "";
        if (!("capikey" in datas)) datas.c_apikey = "";
        if (!("absen" in datas)) datas.absen = {};
      } else
        global.db.data.datas = {
          packname: "",
          author: "",
          linkgc: "",
          api: "",
    ...
    if (opts["gconly"] && !m.chat.endsWith("g.us") && !m.fromMe)
      return conn.reply(
        m.chat,
        "Bot Sedang Dalam Mode Khusus GGonly\nSilahkan Masuk Group Agar Bisa Memakai Bot.\n\n_*[ Link Group ]*_\n( https://chat.whatsapp.com/CTCi5c7jMfSHhBAVKBe3hR )",
        fkontak
      );
    ...
    for (let name in global.plugins) {
      ...
      if ((usedPrefix = (match[0] || "")[0])) {
        ...
        if (plugin.premium && !isPrems && !m.isGroup) {
          // Premium
          fail("premium", m, this);
          continue;
        }
        ...
        if (
          ...
          this.reply(
            m.chat,
            `[❗] Limit kau abis dek, beli melalui *${usedPrefix}buy limit*`,
            m
          );
        ...
        try {
          ...
          if (m.limit) m.reply(+m.limit + " ʟɪᴍɪᴛ ᴛᴇʀᴘᴀᴋᴀɪ ✔️");
        }
        break;
      }
    }
      ...
      if (chat.welcome) {
        ...
        for (let user of participants) {
          ...
          try {
            ...
            let ppmem = pp;
            let ppgc = await this.profilePictureUrl(id, "image");
            let nameusr = await this.getName(user);
            let bg = "https://telegra.ph/file/7f827ca45c833542777f0.jpg";
            ...
            const image = await new knights.Welcome()
              ...
              .setAvatar(ppmem)
              .setBackground(
                "https://telegra.ph/file/aa9ef9dde5ea24dd88476.jpg"
              )
              .toAttachment();
            let wel = await this.resize(image.toBuffer(), 300, 150);
            const imagelea = await new knights.Goodbye()
              .setUsername(nameusr)
              .setGuildName(gcname)
              .setGuildIcon(ppgc)
              .setMemberCount(memcount)
              ...
              .toAttachment();
            let lea = await this.resize(imagelea.toBuffer(), 300, 150);

            this.reply(id, text, fkontak, {
              contextInfo: {
                ...
                externalAdReply: {
                  ...
                  sourceUrl: "https://chat.whatsapp.com/JRfas7SYN1J9btRKiWw8ez",
                },
              },
            });
          }
...
global.dfail = (type, m, conn) => {
  ...
  let msg = {
    rowner: "Printah ini hanya di gunakan oleh *Developer BOT* !",
    owner: "Printah ini hanya di gunakan oleh *Owner* !",
    mods: "Printah ini hanya di gunakan oleh *Moderator BOT* !",
    premium:
      "[ PREMIUM USER ONLY ]\n\n🚩 Menggunakan fitur ini di private chat hanya untuk pengguna premium, upgrade ke paket premium hanya Rp. 10.000,- untuk mendapatkan batas 1K selama 1 bulan.\n\nJika anda ingin membeli paket premium silahkan kirim *.sewabot*",
    ...
    restrict: "Restrict belum di nyalakan di chat ini !",
    unreg:
      "Silahkan daftar untuk menggunakan fitur ini dengan cara mengetik:\n\n*#daftar nama.umur*\n\nContoh: *#daftar Arifzyn.18* !",
  }[type];
  if (msg)
    return conn.reply(m.chat, `Hai ${tag},\n${msg}`, fkontak, {
      mentions: [m.sender],
    });
};
