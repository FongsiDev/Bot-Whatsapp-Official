...
export async function handler(chatUpdate) {
  ...
  try {
    ...
    const groupMetadata =
      (m.isGroup
        ? await Connection.store.fetchGroupMetadata(m.chat, this.groupMetadata)
        : {}) || {};
      ...
      if ((usedPrefix = (match[0] || "")[0])) {
        ...
        if (plugin.level > _user.level) {
          this.reply(
            ...
            `diperlukan level ${
              plugin.level
            } untuk menggunakan perintah ini. Level kamu ${_user.level}`,
            m
          );
...
export async function participantsUpdate({ id, participants, action }) {
  ...
  switch (action) {
    ...
    case "remove":
      if (chat.welcome) {
        let groupMetadata = await Connection.store.fetchGroupMetadata(
          id,
          this.groupMetadata
        );
...
watchFile(file, async () => {
  ...
  if (Connection.reload)
    console.log(await Connection.reload(await Connection.conn));
});
